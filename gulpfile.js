...
function sentinel() {
    ...
    watch(paths.assets.imgs, {ignoreInitial: false}, imgs);
}

function imgs() {
    return src(paths.assets.imgs).pipe(dest(paths.output)) ;
}
...
function scripts() {
    return browserify(paths.scripts.main).transform(babelify.configure({
